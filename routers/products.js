...
router.put(`/:id`, upload_options.single('image') , async (req,res) => {
    ...
    if(!product){
        return res.status(400).send("Not a product id");
    }
    let image_path;
    if(req.file){
        console.log("New file given");
    ...
    if(updated_product){
        ...
        return res.status(400).send("Could not update the product");
    }
